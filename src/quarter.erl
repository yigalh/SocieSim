...
handle_cast({human_died, Who, Pid, HumanFsmState}, State) -> %Who = human record, From = PID of human
  io:format("~p: Human ~p died ~n", [State#state.q_num, Who#humanState.ref]),
...
handle_cast({end_friendship,_Partner1, Partner2}, State) ->
  io:format("~p: Ending friendship ~n",[State#state.q_num]),
...
handle_cast({give_birth, Parent1, Parent2},  State) ->
  ...
  io:format("~p: Baby ~p is born!~n",[State#state.q_num, Baby#humanState.ref]),
...
handle_cast({human_moved_to_quarter, Who},  State) ->
  io:format("~p: Got human ~p to my quarter~n",[State#state.q_num, Who#humanState.ref]),
...
handle_cast({human_request_mate, From, Gender}, State) ->
  ...
  case maps:get(other_gender(Gender), Mate) of
    [] -> io:format("~p: Human added to mating waiting list~n",[State#state.q_num]),
      New = State#state{
      mate = Mate#{
        Gender:=[From|maps:get(Gender, Mate)] }};
    OtherGenderWaiting ->
      io:format("~p: Having a mating match~n",[State#state.q_num]),
...
handle_cast({human_request_friend, From}, State) ->
  case State#state.friend =:= none of
    true  -> io:format("~p: Human waiting for a friend~n",[State#state.q_num]),
      New = State#state{friend = From};
    false -> io:format("~p: 2 Humams are now friends~n",[State#state.q_num]),
...
handle_cast({human_update, Who, Pid}, State) ->
  ...
  case lists:member(Quarter, State#state.q_num) of
    ...
    false ->
      io:format("~p: Sending human to quarter: ~p~n",[State#state.q_num,Quarter]),
